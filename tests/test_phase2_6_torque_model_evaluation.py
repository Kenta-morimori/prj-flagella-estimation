...
from sim_swim.analysis.sweeps import single_flagellum_torque
...
def test_body_motion_summary_reports_displacement_and_speed() -> None:
    summary = single_flagellum_torque._summarize_body_motion(
...
def test_auto_scale_torque_selection_includes_upper_and_first_fail() -> None:
    ...
    selected = single_flagellum_torque._select_auto_scale_torques(rows, torques=torques)

    assert selected == [2.0e-20, 4.0e-20, 1.0e-19]


def test_torque_upper_bound_is_rejected(tmp_path: Path) -> None:
    with pytest.raises(SystemExit, match="1.0e-19"):
        single_flagellum_torque.main(
            [
                "--torques",
                "2.0e-19",
                "--output-dir",
...
def test_phase2_6_eval_smoke_writes_summary(tmp_path: Path) -> None:
