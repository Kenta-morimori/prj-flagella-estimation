...
- Update (2026-05-09): `body_stiffness_scale` をコードのハードコードから除去し、設定パラメータ
  `stiffness_scales.body` として `SimulationConfig` 経由で制御する実装に変更しました。
  デフォルトは論文準拠の `1.0` としています。これによりリポジトリ側の隠れた増強係数を廃止しました。

- 実行結果（トルク診断、duration=1.0s）:
  - CSV: outputs/body_stiffness_diagnostics/body_stiffness_sweep_summary.csv
  - Plot: outputs/body_stiffness_diagnostics/summary_plot.png

  既存の test の一部は旧挙動（body_stiffness_scale=50.0）に依存していたため、テスト側で明示的に
  `stiffness_scales.body=50.0` を指定する形で互換性を保っています。将来的にはテスト期待値を論文準拠に
  更新することを検討してください。

- 解析メモ（2026-05-10 / long-run torque diagnostics）:
  - `outputs/body_stiffness_diagnostics/body_stiffness_sweep_summary.csv` は `duration=1.0s` で実行したため、
    `Simulator.run` 側の仕様により `body_constraint_diagnostics.csv` は出力されていない。
    そのため `shape_pass=false` は body 破綻を意味しない（body_shape_pass 未評価）。
  - `step_summary.csv` の最終行では、3 条件とも `finite_pass=True`、`shape_pass_nonbody=True`、
    `first_fail_category_nonbody=none` であり、非 body 側の破綻は観測されなかった。
  - motor 関連も `motor_degenerate_axis_count=0`、`motor_split_rank_deficient_count=0`、
    `motor_bond_length_clipped_count=0` で、明示的なモータ退化は観測されていない。
  - 位相速度はおおむね `flag_phase_rate_hz ≈ -0.38 ～ -0.43 Hz` で、ゼロ回転ではなく緩い回転が入っている。
    ただし `flag_body_phase_diff_deg` は大きく、回転の見え方は幾何学的位相差と観測窓に依存する。
- 追加メモ（2026-05-17 / short-run body diagnostics と PhaseC 候補比較）:
  - `conf/sim_swim.yaml` と `scripts/01_simulate_swimming/run_motor_scale_sweep.py` の spring 既定値を `H_over_T_over_b=10.0`,
    `s=0.1` に揃え、paper check の spring mismatch を解消した。
  - `scripts/01_simulate_swimming.py` の短時間実行（`duration=0.05s`）では `body_constraint_diagnostics.csv`
    が出力され、body shape の直接判定が可能であることを再確認した。
  - PhaseB2 条件（`n_flagella=1`, `stub_mode=minimal_basal_stub`, `motor.torque_Nm=1.2e-21`, `body=50.0`）では
    `finite_pass=True`, `shape_pass_nonbody=False`, `first_fail_category_nonbody=hook`。
    body diagnostics 末尾は `body_spring_max_stretch_ratio≈0.27`, `body_bend_max_error_deg≈16.9`,
    `body_centerline_max_deviation_um≈0.065` で、body 側は閾値内だった。
  - PhaseC 候補（`motor.torque_Nm=5e-20`, other local scales=1.0, body=1.0）では
    `finite_pass=True`, `shape_pass_nonbody=False`, `first_fail_category_nonbody=hook` に加え、
    body diagnostics 末尾で `body_spring_max_stretch_ratio≈2.76` と body 閾値を超えた。
    したがって、この候補は PhaseC 入りの基準としてはまだ強すぎる。
...
**ルール**: diagnostics CSV / JSON の列を追加・削除・改名した場合は、同一 PR で必ず `docs/PROJECT_PLAN.md` の「Phase 2実行インターフェース（現行コード準拠）」を更新すること。
...
- [ ] `docs/PROJECT_PLAN.md` の Phase2 診断仕様が更新されている
...
### 2. スクリプト実行等価性ルール
`scripts/01_simulate_swimming.py` と `scripts/01_simulate_swimming/run_motor_scale_sweep.py` は同じ Simulator を使用するが、デフォルト設定が異なるため、以下の規則を新たに定めた：
...
### 4. Heatmap 可視化出力の簡素化
[scripts/01_simulate_swimming/plot_motor_scale_collapse_heatmap.py](scripts/01_simulate_swimming/plot_motor_scale_collapse_heatmap.py) を修正し、以下の変更を実施：
...
### 5. 実行コマンド参照ドキュメント
実行コマンドと sweep/可視化の参照は `docs/PROJECT_PLAN.md` の「Phase 2実行インターフェース（現行コード準拠）」へ統合した。
