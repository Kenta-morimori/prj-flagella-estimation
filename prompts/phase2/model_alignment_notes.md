...
### Phase B 検証計画（次のマイルストーン）
- **必須条件**: `paper_table1` + `n_flagella=1` + `motor.torque_Nm>0` + `all projection OFF` + `fd_eps_over_b=1e-3`
- **観測対象**: トルク ON 時に，projection OFF でもなお崩壊や過度な散乱が起きないかを診断
- **評価指標（soft gate）**: `F_torsion_mean_flag`, `F_total_mean_flag`, `flag_bond_len_mean_over_b`, `hook_len_mean_over_b` などの dynamics
- **完了判定**: diagnostics から原因候補を説明可能であること
