...
## 8.1 位置づけ
- 本節は `feature/phase2-2-14` 系で導入した比較実験向けオプションの補足メモである。
- 論文モデルとの差分管理の本体は上記 1-7 節を正とし、ここでは運用上の補助情報のみ記録する。

## 8.2 主な実験オプション
- `projection.enable_flagella_template_projection`
  - template projection の ON/OFF 比較を可能にする。
- `projection.enable_flagella_chain_length_projection_when_template_off`
  - template OFF 時の鎖長拘束を任意で適用する。
- `local_helix.*`
  - template OFF 時に基部近傍の局所ヘリックス形状維持を弱拘束で補助する。
- `steric_exclusion.*`
  - bead-bead 反発（WCA 型）を比較実験用に切り替える。

## 8.3 観測系メモ
- collapse onset の定量観測として `collapse_diagnostics.csv` / `collapse_summary.csv` を利用した。
- 既存観測では `min_interflagella_distance_um` と `min_body_distance_um` の急減が先行指標となる傾向を確認した。