...
- after_axis_weight_final（今回候補）:
  - `local_attach_first_rel_err`: `15950.680%`（改善）
  - `local_first_second_rel_err`: `5163.968%`（悪化）
  - `flag_bond_rel_err_max`: `15606.292%`（微悪化）
  - `local_first_torsion_err_deg`: `15.192`（改善）

### 8.7.4 評価
- `motor_axis_vs_rear_direction_angle_deg` を使う重み関数により、
  previous after 比で `local_attach_first_rel_err` と `local_first_torsion_err_deg` は改善した。
- 一方で `local_first_second_rel_err` と `flag_bond_rel_err_max` は previous after より悪化し、
  Phase C の DoD はまだ未達。
- ただし Phase B 側は明確に改善しており、
  attach-first 側の局所状態依存化が方向として有効であることは確認できた。