#!/usr/bin/env python3
"""Plot Issue #82 hook-overstretch mitigation heatmaps."""
...
def _save_category_heatmap(
    ...
    ax.set_title("Issue #82 first-fail category")
...
def _save_pass_fail_heatmap(
    ...
    ax.set_xlabel(x_name)
    ax.set_ylabel(y_name)
    ax.set_title("Issue #82 shape pass/fail")
    ax.set_xticks(np.arange(len(x_values)))
    ax.set_xticklabels([f"{value:g}" for value in x_values], rotation=30, ha="right")
    ax.set_yticks(np.arange(len(y_labels)))
    ax.set_yticklabels(y_labels)
...
def _save_numeric_heatmap(
    ...
    ax.set_xlabel(x_name)
    ax.set_ylabel(y_name)
    ax.set_title(f"Issue #82 {metric}")
    ax.set_xticks(np.arange(len(x_values)))
    ax.set_xticklabels([f"{value:g}" for value in x_values], rotation=30, ha="right")
    ax.set_yticks(np.arange(len(y_labels)))
    ax.set_yticklabels(y_labels)
