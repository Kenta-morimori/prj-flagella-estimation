...
設定は `conf/sim_swim.yaml` を読みます。一時的な変更は `KEY=VALUE` 形式で指定します。
...
実行時は `[1/3] hook_overstretch ...` のように進捗が標準出力へ表示されます。
...
uv run python scripts/01_simulate_swimming/run_sweep.py \
  ...
  --duration-s 0.001 \
  --torque-nm 0 \
  --first-second-spring-scales 1 \
  --output-dir /private/tmp/phase2_smoke \
  --overwrite
```

profile の既定値は CLI option で上書きできます。sweep の標準 summary は出力先の `summary.csv` です。
...
| `conf/phase2_sweeps/hook_overstretch.yaml` | hook / proximal flagellum 過伸長診断 |
...
uv run python scripts/01_simulate_swimming/plot_heatmap.py \
  ...
  --summary-csv /private/tmp/phase2_smoke/summary.csv \
  --mode first-second-grid \
  --output-dir /private/tmp/phase2_smoke/plots
```
...
| `conf/phase2_sweeps/hook_overstretch_heatmap.yaml` | hook overstretch sweep heatmap |
