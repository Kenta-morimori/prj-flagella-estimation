...
複数条件の sweep は `run_sweep.py` を使います。既存の task-specific sweep は `conf/phase2_sweeps/`、generic multi-run は `conf/phase2_multi_run/` の YAML profile で選びます。
...
generic multi-run では、既存 `conf/sim_swim.yaml` 系 config を base にして、`sweep.axes.*.values` を CLI から差し替えて直積実行できます。campaign 単位で `run.log`、`manifest.json`、`run_manifest.json`、`summary.csv` を残します。
...
| `conf/phase2_multi_run/latest_model_torque_shape_stability.yaml` | 最新モデルで torque を複数条件に振る generic multi-run |
...
generic multi-run の summary plot も `plot_heatmap.py` から行います。`plot.default_y_axis` が未設定の profile では heatmap ではなく 1 軸 line plot を出します。
...
| `conf/phase2_multi_run/latest_model_torque_shape_stability_heatmap.yaml` | generic multi-run summary plot |
