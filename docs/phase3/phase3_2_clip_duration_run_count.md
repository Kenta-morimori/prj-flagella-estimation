...
この文書は Issue #129 の設計メモである。採用する clip 時間長をここで最終決定せず，評価単位，split 規則，軽量 fixture，重い learning curve 実行の渡し方を固定する。
...
| `clip duration` | Phase 4 入力候補 clip の時間長。初期候補は `0.25 s`, `0.5 s`, `1.0 s` | no |
...
| ---: | --- | --- | --- |
| `0.25 s` | primary | `50%` overlap comparison | `>= 1.0 s` pilot; longer run if variance is high |
| `0.5 s` | primary | `50%` overlap comparison | `>= 1.0 s` pilot; longer run if non-overlap count is insufficient |
| `1.0 s` | current baseline | no overlap in current `1.0 s` source | `>= 1.0 s`; longer source needed for multi-window |
...
- `0.25 s` が `0.5 s` / `1.0 s` と比べて class confusion を増やすか。
