...
* [ ] Phase 2.8: RUN固定状態で、べん毛本数による菌体推進・姿勢安定性・軸整列の違いをdatasetとして分析し、Phase 3 handoff baselineを整理（Issue #71 / #125; v1 `n=1,2,3` をtraining candidateとして確定）
...
Phase 2.8では，RUN固定状態でべん毛本数による菌体挙動の違いをdatasetとして評価した。親Issue #71 では，body displacement，speed，body axis angle change，angular velocity，wobble RMS，flag helix axis alignment，cell/flagella axis relation を比較するため，特徴量定義，generic multi-run出力，dataset builder，分布plot，dataset version registryを整備した。Issue #119 では改善モデル dataset v1 を生成し，`n_flagella=1,2,3` は全27 sample strict passかつuser visual review承認済みのtraining candidate，`n=4` はdiagnostic-only，`n>=5` は対象外と判断した。Issue #125 では Phase 3 handoff baselineを v1 RUN固定 `n=1,2,3` とし，2D識別性は #126，共通clip仕様は #127，条件変更の混在規則は #128，clip時間長と独立run数は #129，個体clip pipeline実装は #6 へ分離した。詳細は `docs/phase2/phase2_8_dataset_v1_report.md` と `docs/phase2/phase2_8_phase2_to_phase3_handoff.md` に記録する。
...
* PhaseC（side-attach, dt_star運用）: 実装済み。標準configの `time.dt_star` と必要時の override で内部刻みを制御できる。
...
Phase 3では，実験動画または擬似顕微鏡動画から菌体を検出またはGround Truth passthroughし，個体ごとのクリップ画像列とmetadataを生成する。Phase 2からの初期handoff baselineは dataset v1 の RUN固定 `n_flagella=1,2,3` である。
...
* スケール統一
* bbox / track情報の保存
* 同一 `run_id` / `track_id` 由来clipをsplit groupとして保持

主な対象：

* `src/flagella_estimation/`
* Phase 3 実装時に追加する detection CLI
...
* 実動画経路と擬似動画GT経路が共通のclip / metadata schemaへ収束している。
...
* `src/flagella_estimation/`
* Phase 4 実装時に追加する training / evaluation CLI
