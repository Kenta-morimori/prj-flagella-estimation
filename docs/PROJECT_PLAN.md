...
* [ ] Phase 2.6: single flagellum の bond / bend / torsion 維持策を探索し，螺旋形状維持と持続的な net 回転をmulti-stepで検証（旧定量 gate の誤検出を修正済み、`triplet + hook spring fix` では torque 伝搬が不十分。P2-6-007ではprobe成功まで確認し、完全物理実装は material frame / segment twist の次タスクへ分離）
