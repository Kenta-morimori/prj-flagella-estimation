...
* [ ] Phase 2.6: single flagellum の bond / bend / torsion 維持策を探索し，螺旋形状維持と持続的な net 回転をmulti-stepで検証（旧定量 gate の誤検出を修正済み、`triplet + hook spring fix` では torque 伝搬が不十分。`local_twist_transmission_probe` でroot側orientation activityの先端側伝搬とnet回転を確認）
* [ ] Phase 2.7: 複数べん毛条件で形状崩壊しないことを検証（`distributed_flagellum` の複数べん毛非崩壊確認を含む）
* [ ] Phase 2.8: 複数べん毛条件で後方束化挙動を検証
* [ ] Phase 2.9: べん毛回転による菌体の遊泳挙動を検証
...
Phase 2.6では，従来の `flag_phase_rate_hz` と `median(abs(flag_helix_spin_rate_hz))` が目視の螺旋スピンと一致しないことを確認した。`outputs/phase2_6_dt1e4_spin_review/2026-05-31/230124/` では `median_abs_flag_helix_spin_rate_hz=24.40` だったが，`flag_helix_spin_phase_deg` の累積差は 0.25 s で 0.501 deg (`net_abs_flag_helix_spin_revolutions=0.00139`) しかなく，ユーザー目視でもほぼ回転していなかった。そのため Phase 2.6 gate は，瞬間速度だけでなく `net_abs_flag_helix_spin_revolutions` と `flag_helix_spin_direction_consistency` を hard gate に含める。さらに，既存 topology に存在する `attach-first` hook spring が時間積分で未使用だった実装漏れを修正し，診断用に `motor.force_distribution=distributed_flagellum` を追加した。以後の実行はデフォルト設定を変えず，CLI override で `time.dt_star=1.0e-4` を明示する。`triplet + hook spring fix` では root 方位の net 回転が螺旋全体の net 回転へ十分に伝わらず，現行 bead-position-only model に material frame / segment twist / axial torque flux がないことが主因候補である。P2-6-007では実験用 `motor.force_distribution=local_twist_transmission_probe` を追加し，segment orientation state として root 側に入れた activity が先端側へ部分伝搬することを確認した。代表条件 `torque=2.0e-20 N m`, `dt_star=1.0e-4`, `local_spring_scale=1.2` では，0.5 s / 5000 steps で `net_abs_flag_helix_spin_revolutions=1.04745`, `flag_helix_spin_direction_consistency=0.81507`, `local_twist_tip_activity_ratio=0.34836` を示し，shape gate を通過する。ただし bead force 変換はまだ近似であり，完全な material frame / segment twist 物理モデルではない。多本べん毛，束化，遊泳軌跡，2D動画自然さはPhase 2.7以降で扱う。詳細は `docs/phase2/phase2_6_helix_retention_gate.md`, `docs/phase2/phase2_6_triplet_twist_dof_design.md`, `docs/adr/0001_phase2_distributed_flagellar_torque.md`, `docs/adr/0002_phase2_axial_torque_flux_probe.md`, `docs/adr/0003_phase2_local_twist_transmission.md` に記録する。

### Phase 2運用フェーズ（A/B/C/D）と現状

`prompts/phase2/0037.md` の運用フェーズは，本ドキュメントへ統合して管理する。
...
`prompts/phase2/0037.md` の PhaseD 要件に対する確認結果:
...
本ファイルは，従来の `docs/00_overview.md` の内容を統合する親ドキュメントである。
...
* `docs/adr/0001-use-codex-cli.md`
* `docs/adr/0002-separate-development-and-review-agents.md`
* `prompts/agents/development_agent.md`
* `prompts/agents/review_agent.md`
* `prompts/agents/adr_agent.md`
* `.codex/schemas/review_result.schema.json`
* `tools/codex/run_codex_task.sh`
