...
* [ ] Phase 2.6: single flagellum の bond / bend / torsion 維持策を探索し，螺旋形状維持をmulti-stepで検証（定量 gate PASS、ユーザー定性レビュー待ち）
...
Phase 2.6では，従来の `flag_phase_rate_hz` が目視の螺旋スピンと一致しないことを確認し，body追従座標系で推定する `flag_helix_spin_rate_hz` を hard gate に追加した。`4.0e-21 N m`, `dt_star=2.5e-4`, `local_hook_scale=8`, `local_spring_scale=5`, `local_bend_scale=8`, `local_torsion_scale=4` は形状を保つが螺旋スピンがほぼ出ないため fail とする。以後の実行はデフォルト設定を変えず，CLI override で `time.dt_star=1.0e-4` を明示する。現時点の長時間代表は `2.5e-20 N m`, `dt_star=1.0e-4`, `local_hook_scale=4`, `local_spring_scale=2`, `local_bend_scale=2`, `local_torsion_scale=2` で，0.25 s / 2500 steps の定量 gate を通過した。これは単一べん毛・決定論的条件・定量 gate 上の確認であり，ユーザー定性レビューは未完了である。多本べん毛，束化，遊泳軌跡，2D動画自然さはPhase 2.7以降で扱う。詳細は `docs/phase2/phase2_6_helix_retention_gate.md` に記録する。
