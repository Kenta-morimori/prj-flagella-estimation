...
* [ ] Phase 2.6: single flagellum の bond / bend / torsion 維持策を探索し，螺旋形状維持と持続的な net 回転をmulti-stepで検証（旧定量 gate の誤検出を修正済み、`triplet + hook spring fix` では torque 伝搬が不十分。`axial_torque_flux_probe` で有効な近似伝搬を確認）
...
Phase 2.6では，従来の `flag_phase_rate_hz` と `median(abs(flag_helix_spin_rate_hz))` が目視の螺旋スピンと一致しないことを確認した。`outputs/phase2_6_dt1e4_spin_review/2026-05-31/230124/` では `median_abs_flag_helix_spin_rate_hz=24.40` だったが，`flag_helix_spin_phase_deg` の累積差は 0.25 s で 0.501 deg (`net_abs_flag_helix_spin_revolutions=0.00139`) しかなく，ユーザー目視でもほぼ回転していなかった。そのため Phase 2.6 gate は，瞬間速度だけでなく `net_abs_flag_helix_spin_revolutions` と `flag_helix_spin_direction_consistency` を hard gate に含める。さらに，既存 topology に存在する `attach-first` hook spring が時間積分で未使用だった実装漏れを修正し，診断用に `motor.force_distribution=distributed_flagellum` を追加した。以後の実行はデフォルト設定を変えず，CLI override で `time.dt_star=1.0e-4` を明示する。現時点の定量代表は `torque=2.0e-20 N m`, `dt_star=1.0e-4`, `motor.force_distribution=distributed_flagellum`, `local_spring_scale=1.2` で，0.5 s / 5000 steps において `net_abs_flag_helix_spin_revolutions=1.0416` を示し，形状 gate を通過する。一方で，`triplet + hook spring fix` では root 方位の net 回転が螺旋全体の net 回転へ十分に伝わらず，現行 bead-position-only model に material frame / segment twist / axial torque flux がないことが主因候補である。P2-6-007では実験用 `motor.force_distribution=axial_torque_flux_probe` を追加し，root bead を原点とする軸方向 torque flux 近似が 0.5 s / 5000 steps で `net_abs_flag_helix_spin_revolutions=1.08546` と shape gate PASS を満たすことを確認した。ただしこれは真の material frame / segment twist 伝搬ではないため，短期的な有効近似として扱う。多本べん毛，束化，遊泳軌跡，2D動画自然さはPhase 2.7以降で扱う。詳細は `docs/phase2/phase2_6_helix_retention_gate.md`, `docs/phase2/phase2_6_triplet_twist_dof_design.md`, `docs/adr/0001_phase2_distributed_flagellar_torque.md`, `docs/adr/0002_phase2_axial_torque_flux_probe.md` に記録する。
