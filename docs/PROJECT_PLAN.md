...
* [ ] Phase 2.7: 複数べん毛条件で、崩壊せず後方束化する条件帯を検証（hook角度・トルク・本数を段階評価）
* [ ] Phase 2.8: べん毛回転による菌体の遊泳挙動を運動指標で検証（姿勢変化・揺れ・束軸と菌体軸のずれを含む）
* [ ] Phase 2.9: 3D/2D動画出力をレビュー・MLデータ作成向けに整備（`fps_out_3d` を含む）
