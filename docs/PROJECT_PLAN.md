...
Phase 2.6では，`4.0e-21 N m` の single full flagellum 条件に対して，`dt_star=2.5e-4` と `local_bend_scale=8` の組み合わせを multi-step helix retention representative として固定した。`dt_star` 縮小だけでは shape は維持できるが `flag_phase_rate_hz` が落ちるため，安定回転条件としては不十分である。詳細は `docs/phase2/helix_retention_gate.md` に記録する。
