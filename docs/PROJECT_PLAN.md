...
* [ ] Phase 2.5: body + hook + single flagellum のトルクあり安定回転を検証
* [ ] Phase 2.6: single flagellum の螺旋形状維持をmulti-stepで検証
...
* PhaseD（sweep再開）: D2/D3は実装済み，D1（基準トルク再同定）は未完。
...
1. D1: B2を満たす基準トルク再同定  
未完。現行テストでは `torque=1.2e-21` 条件が break ではなく安定通過する想定に更新されており，旧基準のままでは運用できない。
