...
Phase 2.6では，`4.0e-21 N m` の single full flagellum 条件に対して，`dt_star=2.5e-4`, `local_hook_scale=8`, `local_spring_scale=5`, `local_bend_scale=8`, `local_torsion_scale=4` の組み合わせを multi-step helix retention representative として固定した。`dt_star` 縮小や `local_bend_scale=8` 単独では shape は維持できても `flag_phase_rate_hz` が落ちるため，安定回転条件としては不十分である。CIでは200-step representativeを固定し，同条件のローカル1000-step probe (`duration_s=0.25`) でも定量 gate を通過することを確認した。これは単一べん毛・決定論的条件・定量 gate 上の確認であり，ユーザー定性レビューは未完了である。多本べん毛，束化，遊泳軌跡，2D動画自然さはPhase 2.7以降で扱う。詳細は `docs/phase2/phase2_6_helix_retention_gate.md` に記録する。
