...
- 合成トラッキング検証（tests/test_synthetic_tracking.py）は本番と同じ `detect_frame` / `Tracker` を通し、生成したGT軌跡と対応付けて評価すること。ID継続を厳密に見るため、GTごとにカバー率>=95%のトラックが存在しない場合は失敗とする。オーバーレイも対応後のラベルと軌跡で描画する。
- overlayでは同一IDの軌跡を履歴として描画する設定を用意し、非表示にする閾値もconfigで切り替え可能にすること。スケールバーは菌体短辺長に基づき常に描画する。
