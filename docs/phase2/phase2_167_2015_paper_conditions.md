...
実装する。120-bead geometry生成は#166、長時間安定性とprofileのsupported昇格判定は
#168で扱う。このため2015 project / paper profileはparse・正規化・manifest出力のみ
可能で、simulation実行は引き続き拒否する。

## Profile status

| profile | dynamics | geometry | simulation | motor model |
| --- | --- | --- | --- | --- |
| `sim_swim_2015.yaml` | implemented | pending | blocked | `root_torque_segment_couples` |
| `sim_swim_2015_paper.yaml` | implemented | pending | blocked | `hook_coupled_body_reaction` |
...
| `body.prism.n_prism` | `6` | figure、30 body beads、5 layersからのproject推論 | geometry pending #166 |
...
| Hook length | `0.25 b` | implementation assumption | geometry pending #166 |
| body diagonal brace | OFF | paper condition | geometry pending #166 |
| center-layer attachment slots | 3 | figure-based geometry interpretation | geometry pending #166 |
...
局所supportが縮退した場合だけ全body beadsへfallbackする。2010 triangular geometryを
使った短時間検証ではsupportは各motor 5 beads、fallbackなしで、swimmer全体のnet forceと
net torqueは数値許容範囲内だった。
...
上記が完了するまで、次の2015 simulation commandはoutput作成前に意図的に拒否される。
