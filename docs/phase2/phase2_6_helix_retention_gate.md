...
したがって、単一べん毛については、定量 gate の範囲で「5000-step までの螺旋形状維持と net 1回転以上」をprobe条件で確認済みである。ただし、これは単一べん毛・決定論的条件・診断用 motor 近似上の確認であり、完全物理実装の達成ではない。完全物理実装では、material frame / segment twist / 局所 force couple により非局所force injectionを避ける必要がある。

## 定性レビュー

本PRのprobe成功判定は定量gateで行う。完全物理実装としてのPhase 2.6完了判定は、material frame / segment twist の次タスクで改めて行う。
