...
現時点では、螺旋形状維持と net 1回転以上を両立する pass representative は未確立である。
...
CI では runtime を抑えるため、`duration_s=0.05`, `dt_star=1.0e-4` の 500-step 代表条件で「瞬間スピン rate が高くても net 回転がなければ fail」となることを固定する。ローカル確認では、`torque=2.5e-20 N m`, `dt_star=1.0e-4`, `local_hook_scale=4`, `local_spring_scale=2`, `local_bend_scale=2`, `local_torsion_scale=2` 条件で `duration_s=0.25` の 2500-step probe が新 gate で fail することを確認した。

したがって、単一べん毛については、上記の定量 gate の範囲で「2500-step までの螺旋スピンを伴う安定的な回転」はまだ確認できていない。次は motor torque が root 方位の揺れや局所変形ではなく、螺旋全体の累積回転へ伝達されるかを診断・修正する。
