...
| `torque=2.5e-20`, `dt_star=1.0e-4`, `local_hook_scale=4`, `local_spring_scale=2`, `local_bend_scale=2`, `local_torsion_scale=2` | pass | 0.25 s / 2500 steps で `median_abs_flag_helix_spin_rate_hz=24.40`。0.25 s で約6.1回転相当 |
...
CI では runtime を抑えるため、`duration_s=0.05`, `dt_star=1.0e-4` の 500-step 代表条件を固定する。ローカル確認では、`torque=2.5e-20 N m`, `dt_star=1.0e-4`, `local_hook_scale=4`, `local_spring_scale=2`, `local_bend_scale=2`, `local_torsion_scale=2` 条件で `duration_s=0.25` の 2500-step probe も pass することを確認した。

したがって、単一べん毛については、上記の定量 gate の範囲で「2500-step までの螺旋スピンを伴う安定的な回転」を確認済みである。ただし、これは単一べん毛・決定論的条件・現行診断 gate 上の確認であり、ユーザー目視による定性検証は未完了である。長時間の実用動画生成や多本べん毛条件での安定性を保証するものではない。
