...
- result:
  ...
  - PR #95 の数値上の暫定候補は `motor.local_attach_frame_position_scale=2`, `motor.local_attach_frame_tangent_scale=1.5`, `motor.local_first_second_spring_scale=1`, `motor.torque_Nm=1.9e-20` とする。ただし低 torque では十分に回転・束化・遊泳しない可能性があるため，代表条件として採用するには後方束化条件の動画定性評価を必須とする。
  - 定性評価の観点は，後方へ揃えた初期べん毛軸，滑らかな回転，後方束化または後方軸整列，遊泳らしい移動，hook/root/flagella/body の目立つ破綻なし，2D projection の自然さである。side-attach 動画や自動 shape gate だけでは PR #95 の代表条件採用を完了扱いにしない。
...
- user-run commands:
  ...
  - PR #95 merge前の後方束化動画定性評価:
    `uv run python -m scripts.01_simulate_swimming flagella.initial_helix_axis_from_rear_deg=0 seed.attach_seed=0 seed.phase_seed=0 time.duration_s=0.6 motor.torque_Nm=1.9e-20 motor.local_attach_frame_position_scale=2 motor.local_attach_frame_tangent_scale=1.5 output_sampling.out_all_steps_3d=false render.render_flagella_2d=true render.show_flagella_helix_axis_3d=true output.base_dir=outputs/phase2_94/posterior_merge_review/qual_videos/fp2_ft1p5_torque1p9_dur0p6`
