...
| P2-D24 | v1 r1 n=3 failure診断をv1 r2 5 s stabilityで置換する | adopted |
...
### P2-D24: v1 r1 n=3 failure診断をv1 r2 5 s stabilityで置換する
...
- **Interpretation:** v1 r1のfailureは現行のv1 r2 campaignに対する物理failure gateではない。非定常回転とattach seed差は，physical failureではなく、canonical model freeze後にPhase 3で評価するwithin-class variationとして扱う。
- **Decision:** Issue #158はv1 r2 5 s stability campaignにより置換済みとして完了する。dataset v2とIssue #205のblockerから外す。旧診断文書は履歴・探索的根拠として保持し，physical model変更の根拠には用いない。
