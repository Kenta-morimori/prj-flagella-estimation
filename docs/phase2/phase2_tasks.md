...
- result:
  ...
  - 36条件を1枚へ詰めると各 panel が読みにくいため，`replay.max_panels_per_grid` による page 分割出力を追加した。`flagella_count_behavior_diagnostic.yaml` は最大9 panel/page とし，`grid_swim3d_page01.mp4` などを生成する。
...
- summary:
  ...
  - canonical v0 `dataset_id` は `v0`，canonical output は `outputs/phase2_analysis/flagella_count_behavior/datasets/v0` とし，dataset 条件は `metadata.dataset_scope` と registry で追う方針にした。
...
- acceptance criteria:
  ...
  - [x] clip時間長・データ数の後続評価へ渡す観測時間スケールが整理されている。
- verification:
