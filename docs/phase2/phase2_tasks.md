...
- result:
  ...
  - 2026-06-30追補として，`af=1, axis=1, fs=1, fp=2, ft=1.5` 固定で torque を `1.75e-20, 1.9e-20, 2.0e-20, 2.25e-20, 2.5e-20` に切り分けた。0.6 s の自動 shape gate は `1.9e-20` のみ pass した。`1.75e-20` は final 近傍の local `4-5` で border fail，`2.0e-20` 以上は local `3-4` の flag bond 過伸長で fail した。
  - PR #95 の数値上の暫定候補は `motor.local_attach_frame_position_scale=2`, `motor.local_attach_frame_tangent_scale=1.5`, `motor.local_first_second_spring_scale=1`, `motor.torque_Nm=1.9e-20` とする。ただし低 torque では十分に回転・束化・遊泳しない可能性があるため，代表条件として採用するには動画の定性評価を必須とする。
  - 定性評価の観点は，滑らかな回転，束化，遊泳らしい移動，hook/root/flagella/body の目立つ破綻なし，2D projection の自然さである。自動 shape gate だけでは PR #95 の代表条件採用を完了扱いにしない。
...
- user-run commands:
  - `fp=2,3` の代表比較と `fp=2` の torque 追加切り分けを再実行する場合は，`first-second-grid` の script default に引きずられないよう `af=1, axis=1` を明示する:
    `set -e; for fp in 2 3; do uv run python scripts/01_simulate_swimming/run_phase2_82_hook_overstretch_sweep.py --mode first-second-grid --duration-s 0.6 --attach-seed 0 --phase-seed 0 --fixed-attach-first-spring-scale 1 --fixed-body-axis-angle-scale 1 --fixed-attach-frame-position-scale "$fp" --fixed-attach-frame-tangent-scale 1.5 --first-second-spring-scales 1 --output-dir "outputs/phase2_94/representative_af1_axis1_fp${fp}_ft1p5_fs1_torque2p5e20_dur0p6" --overwrite --progress-interval 5000; done; for torque in 1.75e-20 1.9e-20 2.0e-20 2.25e-20 2.5e-20; do label=$(printf "%s" "$torque" | sed 's/\\.//g; s/e-20/e20/'); uv run python scripts/01_simulate_swimming/run_phase2_82_hook_overstretch_sweep.py --mode first-second-grid --duration-s 0.6 --attach-seed 0 --phase-seed 0 --fixed-attach-first-spring-scale 1 --fixed-body-axis-angle-scale 1 --fixed-attach-frame-position-scale 2 --fixed-attach-frame-tangent-scale 1.5 --first-second-spring-scales 1 --torque-nm "$torque" --output-dir "outputs/phase2_94/torque_${label}_af1_axis1_fs1_fp2_ft1p5_dur0p6" --overwrite --progress-interval 5000; done`
  - 共有対象は `outputs/phase2_94/representative_af1_axis1_fp2_ft1p5_fs1_torque2p5e20_dur0p6/phase2_82_hook_scale_sweep_summary.csv`, `outputs/phase2_94/representative_af1_axis1_fp3_ft1p5_fs1_torque2p5e20_dur0p6/phase2_82_hook_scale_sweep_summary.csv`, `outputs/phase2_94/torque_*_af1_axis1_fs1_fp2_ft1p5_dur0p6/phase2_82_hook_scale_sweep_summary.csv`。
  - PR #95 merge前の定性評価:
    `uv run python -m scripts.01_simulate_swimming seed.attach_seed=0 seed.phase_seed=0 time.duration_s=0.6 motor.torque_Nm=1.9e-20 motor.local_attach_frame_position_scale=2 motor.local_attach_frame_tangent_scale=1.5 output_sampling.out_all_steps_3d=false render.render_flagella_2d=true render.show_flagella_helix_axis_3d=true output.base_dir=outputs/phase2_94/qual_review_fp2_ft1p5_torque1p9_dur0p6`
  - 定性評価の共有対象は，run directory の `manifest.json`, `render/swim3d.mp4`, `render/swim3d_final.png`, `render2d/projection.mp4`, `sim/step_summary.csv`。
- docs:
  - `docs/phase2/phase2_current.md`
  - `docs/phase2/phase2_tasks.md`
