...
| `v1` | `r2` | `nf1_2_3_4_as3_ps3_dur2p0` | planned; user-run | `integrated_3d_2d_feature_study` | pending feature review; `n=4` diagnostic-only | `issue199_tau_linked_T2p5e-20_dt1e-3_body1` | `conf/phase2_multi_run/flagella_count_behavior_v1_r2_tau_linked_2s.yaml` | `v1_r2_tau_linked_2s` | `not_generated` | `outputs/phase2_multi_run/flagella_count_behavior_v1_r2` | Issue #199選定条件の2秒・36 source run。物理条件がv1 r0と異なるため、既存training baselineを置換せずfeature比較後に採否を判断する。 |
...
v1 は #115/#116 のモデル改善結果を受けて #119 で作成する。v1 config は原則として `conf/phase2_multi_run/flagella_count_behavior_v1.yaml` とし，dataset_id は `v1` とする。#116 の候補評価 profile は `conf/phase2_multi_run/flagella_count_stability_narrow_seed00.yaml` と `conf/phase2_multi_run/flagella_count_stability_smoke_seed00.yaml` である。
