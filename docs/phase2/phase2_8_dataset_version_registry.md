...
| `v1` | `r0` | TBD | planned | `planned` | TBD | TBD after #116 | TBD | TBD | `not_generated` | none | #116 の seed固定狭域 sweep と n=1,2,3 smoke check 後に #119 で再生成可否を判断する。現時点では v1-ready 条件は未確定。 |
...
v1 は #115/#116 のモデル改善結果を受けて #119 で作成する。v1 config は原則として `conf/phase2_multi_run/flagella_count_behavior_v1.yaml` とし，dataset_id は `v1` とする。#116 の候補評価 profile は `conf/phase2_multi_run/flagella_count_stability_narrow_seed00.yaml` と `conf/phase2_multi_run/flagella_count_stability_smoke_seed00.yaml` である。#115 の `flag_spring/body` 比較では `n>=4` を直接戻せる安定条件が出ておらず，#116 の長時間 run も未実行のため，現時点では v1 を生成しない。
