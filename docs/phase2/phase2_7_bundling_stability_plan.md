...
- `time.dt_star=1.0e-4`
- `duration_s>=0.5`
- `flagella.initial_tangent_vs_rear_deg=10`
...
- `flagella.initial_orientation_mode` を追加した。
  ...
  - `posterior_aligned`: 診断用。hook付着は側面のまま、べん毛初期接線を菌体後方へ向ける。
- `flagella.initial_tangent_vs_rear_deg` を追加した。指定時は `initial_orientation_mode` より優先し、0度を菌体後方軸、90度を従来の側面付着相当として扱う。
- `step_summary.csv` に束化・遊泳指標を追加した。
- `scripts/01_simulate_swimming/run_phase2_7_bundling_sweep.py` を追加し、`n_flagella`, `torque_Nm`, `initial_tangent_vs_rear_deg` のsweep結果を `phase2_7_bundling_sweep_summary.csv` へ集約できるようにした。
