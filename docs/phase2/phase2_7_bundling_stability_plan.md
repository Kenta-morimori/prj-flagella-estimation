...
今回の最優先は、後方寄せ初期条件で多べん毛が形状崩壊せず動くか、さらに束化または部分束化へ向かうかを、`step_summary.csv` と sweep summary で再現可能にすることである。束化が確認できない場合も、`no_bundle_drive`, `hook_drift`, `repulsion_dominated` など原因を分けて次タスクへ渡す。
