...
- `time.dt_star=1.0e-4`
- `duration_s>=0.5`
- `flagella.initial_flagellum_axis_from_rear_deg=10`
- `motor.local_*_scale=1.0`
- `n_flagella=3,6,9` を主対象とし、必要に応じて `n_flagella=1` を比較基準にする。

分類は二値判定にしない。`posterior_bundle`, `partial_bundle`, `no_bundle`, `collapse` を区別し、1本だけ独立する状態は失敗ではなく `partial_bundle` として記録する。

今回の最優先は、後方寄せ初期条件で多べん毛が形状崩壊せず動くか、さらに束化または部分束化へ向かうかを、`step_summary.csv` と sweep summary で再現可能にすることである。hook角度そのものは目的指標ではなく、後方束化を観察するために破綻してはいけない制約として扱う。束化が確認できない場合も、`no_bundle_drive`, `hook_drift`, `repulsion_dominated` など原因を分けて次タスクへ渡す。
...
2. hook角度または初期べん毛軸方向を調整し、全べん毛を菌体後方寄りへ向けた条件を作る。
3. 後方寄り初期べん毛軸条件で安定トルク帯を再探索し、後方束化候補を分類する。
...
目標は、各べん毛の初期べん毛軸が菌体長軸の後方成分を持つことである。完全に同じ軸へ重ねるのではなく、付着点と螺旋位相の差は残す。これにより、後方束化しやすい条件を作りつつ、べん毛間干渉や部分束化も観察できるようにする。

幾何の意図は、`attach -> first` を菌体長軸に対してほぼ垂直な外向き方向に保ち、`first -> second` 以降のべん毛軸を後方寄りへ向けることである。したがって、`hook.threshold_deg=90` は後方束化の目的変数ではなく、hookが潰れすぎないための制約である。

2026-06-04時点では、後方軸からの角度を連続値として扱う方針にする。

- `flagella.initial_flagellum_axis_from_rear_deg=0`: 菌体後方軸と平行。旧 `posterior_aligned` 相当。
- `flagella.initial_flagellum_axis_from_rear_deg=10`: 主候補。ほぼ後方向きだが、完全平行ではなく少し外側へ開く。
- `flagella.initial_flagellum_axis_from_rear_deg=90`: 従来 `side_attach` 相当。

このconfig名は、初期状態で「べん毛軸」が「菌体後方軸」から何度ずれているかを明示するための名前である。PR #60 の初期実装名 `initial_tangent_vs_rear_deg` は旧aliasとして一時的に受けるが、以後のドキュメント・実行コマンドでは `initial_flagellum_axis_from_rear_deg` を使う。
...
## 2026-06-09 main派生ブランチでの実装・診断結果
...
- `flagella.initial_orientation_mode` を追加した。
  - `side_attach`: 既定値。従来どおり、側面付着方向へ初期べん毛を伸ばす。
  - `posterior_aligned`: 診断用。hook付着は側面のまま、べん毛初期軸を菌体後方へ向ける。
- `flagella.initial_flagellum_axis_from_rear_deg` を追加した。指定時は `initial_orientation_mode` より優先し、0度を菌体後方軸、90度を従来の側面付着相当として扱う。
- `step_summary.csv` に束化・遊泳指標を追加した。
- `scripts/01_simulate_swimming/run_phase2_7_bundling_sweep.py` を追加し、`n_flagella`, `torque_Nm`, `initial_flagellum_axis_from_rear_deg` のsweep結果を `phase2_7_bundling_sweep_summary.csv` へ集約できるようにした。
...
初期geometryでは、`initial_flagellum_axis_from_rear_deg=10` の条件で期待通りの配置になっていた。
...
`n_flagella=3`, `flagella.initial_flagellum_axis_from_rear_deg=10`, `time.dt_star=1.0e-4` で、strict gate と hook長relaxed gate の差分を評価した。
...
1. `initial_flagellum_axis_from_rear_deg=10`
2. `initial_flagellum_axis_from_rear_deg=30`
3. `initial_flagellum_axis_from_rear_deg=0`
4. `initial_flagellum_axis_from_rear_deg=60`
5. `initial_flagellum_axis_from_rear_deg=90`
...
- `flagella.initial_flagellum_axis_from_rear_deg` を追加した。指定時は `initial_orientation_mode` より優先し、0度を菌体後方軸、90度を従来 `side_attach` 相当として扱う。
...
- `initial_flagellum_axis_from_rear_deg = 0, 10, 30, 60, 90`
...
| initial_flagellum_axis_from_rear_deg | shape_pass_nonbody | first_fail_category_nonbody | net_abs_flag_helix_spin_revolutions | hook_len_rel_err_max | bundle_axis_vs_rear_angle_deg | bundle_participation_ratio | flag_flag_close_pair_count | 判定 |
| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
...
`initial_flagellum_axis_from_rear_deg=10`, `duration_s=0.5`, `motor.local_spring_scale=1.2` で torque を上げると、束化より先にhookが破綻した。
