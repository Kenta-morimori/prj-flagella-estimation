...
## 要約
- どの sweep でも `pos_all_finite=True`, `any_nan=False`, `any_inf=False` を維持し、`finite_pass=True` だった。
- `motor_degenerate_axis_count`, `motor_split_rank_deficient_count`, `motor_bond_length_clipped_count` は全ケースで 0 のままだった。
- Smoke test（1ms）では `shape_pass_nonbody=True`, `body_shape_pass=True`, `shape_pass=True`, `first_fail_category=none` だった。
- Main observation（50ms）では全ケースで `shape_pass_nonbody=False`, `body_shape_pass=False`, `shape_pass=False` となり、`first_fail_category=hook` だった。
- 50ms では形状誤差が悪化し、特に `local_first_second_rel_err` と `flag_bond_rel_err_max` が先に増えた。
...
- `pos_all_finite=True`
- `finite_pass=True`, `shape_pass_nonbody=False`, `body_shape_pass=False`, `shape_pass=False`
...
### 1. 破綻の先行点
- NaN/Inf や退化カウンタ増加は出なかった（`finite_pass=True`, 退化カウンタ=0）。
- 一方で 50ms では gate 判定上の first-fail が全ケースで `hook` になった。
- 連続量としては `local_first_second_rel_err` と `flag_bond_rel_err_max` の悪化が顕著で、`local_hook_scale=0.0` で急増した。
...
### 4. 実行上の補足
- 1ms の smoke sweep は shape gate も通過した。
- 50ms の main observation は全ケースで shape gate 不通過（`first_fail_category=hook`）となり、`pos_all_finite` 単独判定との差が明確になった。
- 次に掘るなら `local_spring_scale` または `local_bend_scale` / `local_torsion_scale` を順に落とすのが自然。

## 気づいた点
- `pos_all_finite` は数値健全性の条件であり、形状維持の合否とは一致しないことが確認できた。
