...
Issue #54 では、`root_torque_segment_couples` 導入後の単一べん毛モデルについて、高トルク条件での形状安定性と local stiffness scaling の必要性を定量評価する。

Issue #88 以降、旧名 `material_twist_local_couple` は `root_torque_segment_couples` の deprecated alias、旧名 `distributed_flagellum` は `root_torque_axis_projection` の deprecated alias として扱う。診断用 probe mode `axial_torque_flux_probe` / `local_twist_transmission_probe` は削除済みであり、新規評価条件には含めない。
...
その後、probe 系 mode で「トルクが螺旋全体へ届けば綺麗に回る」ことを診断し、P2-6-008 では `root_torque_segment_couples` を採用した。代表条件では、`motor.torque_Nm=2.0e-20`, `time.dt_star=1.0e-4`, `duration_s=0.5`, `motor.local_spring_scale=1.2` で shape gate PASS と net 1回転以上を確認した。
...
- 原則として必須比較は置かず、`root_torque_segment_couples` を単独で評価する。
- `root_torque_axis_projection` は、評価結果の解釈に詰まった場合の追加診断に限定する。
