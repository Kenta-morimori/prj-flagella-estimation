...
* Phase 2 / Issue #100: `conf/phase2_multi_run/` に generic multi-run profile を追加し，`run_sweep.py` から `kind=generic_multi_run` を実行できるようにした。base simulation config に対して `sweep.axes.*.values=[...]` や通常の simulation override を CLI で重ねて直積実行でき，campaign 単位で `run.log` / `manifest.json` / `run_manifest.json` / `summary.csv` を残す。`plot_heatmap.py` は同 profile の summary から 1軸 line plot または 2軸 heatmap を生成でき，`render_shape_stability_grid_replay.py` は generic campaign の `condition_order` / `condition_label` / `output_dir` を読んで再シミュレーションなし replay を行える。標準 profile は `latest_model_torque_shape_stability` とし，最新モデルの torque 複数条件比較を 3 コマンドで回せる。
