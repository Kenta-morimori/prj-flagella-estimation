...
* Phase 2.8 / Issue #116 planning: #115 の改善域に絞り，seed固定 `n=4,5,6` の少数 sweep profile `conf/phase2_multi_run/flagella_count_stability_narrow_seed00.yaml` を使う。軸は `stiffness_scales.flag_spring=[1.75,2.0,2.25]` と `stiffness_scales.body=[1.0,1.5,2.0,2.5]`。候補が出た後だけ `conf/phase2_multi_run/flagella_count_stability_smoke_seed00.yaml` で `n=1,2,3` smoke check を行う。#116 の実行・判定手順は `docs/phase2/phase2_8_issue116_stability_sweep_plan.md` を読む。長時間 run は Codex では実行せず，user実行結果で #119 へ進むか判断する。
