...
* #82 の attach-frame 補強は hook過伸長を 0.5 s 代表条件で抑えるが，長時間では flagellum bond 過伸長が支配的になる。全 local scale の default は `1.0` に据え置き，`fp=3, ft=1.5` は診断候補として扱う。出力整理では `step_summary.csv`，`trajectory.csv`，`state_archive.npz`，`manifest.json` を再現用として保持し，途中停止run・重複run・報告に使わない再生成可能な動画/frameだけを削除対象にする。
