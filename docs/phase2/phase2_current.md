...
* Phase 2.8 / Issue #113 follow-up ordering: #113 / PR #114 で failure mode を確定し，#117/#118 で v0 統計レポートと `model_id` 命名規則を完了した。次は #115 で `n>=4` の flag bond 過伸長 / body_spring failure をモデル修正し，#116 の少数sweep / heatmap 方針を経て，#119 で改善モデル dataset v1 を再生成する。#17 は背景ノイズ関連のためこの関係整理から除外する。
* Phase 2.8 / Issue #115: `stiffness_scales.flag_spring` を追加し，hook ではなく flag 内 spring を強める project-specific stabilizing extension として candidate 比較できるようにした。default は `1.0` で既存 baseline には明示しない。seed固定 candidate profile は `conf/phase2_multi_run/flagella_count_stability_candidates_seed00.yaml` で，`n_flagella=[4,5,6]`，`flag_spring=[1.0,1.5,2.0]`，`body=[1.0,2.0]` を比較する。長時間 run は未実行で，`uv run python scripts/01_simulate_swimming/run_multi_run.py config=conf/phase2_multi_run/flagella_count_stability_candidates_seed00.yaml overwrite=true` をユーザー実行後に #116/#119 へ渡す候補を判断する。
