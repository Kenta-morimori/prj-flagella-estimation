...
* Phase 2.8 / Issue #113: dataset v0 で見えた `n>=4` の破綻境界確認に向け，`conf/phase2_multi_run/flagella_count_failure_boundary_seed00.yaml` を追加した。`attach_seed=0`, `phase_seed=0`, `n_flagella=4,5,6` の実行結果では全条件が `flag` first fail となり，first fail は `n=4: 0.3273 s`, `n=5: 0.3168 s`, `n=6: 0.3505 s`，max flag bond rel err は `1.8486`, `1.5037`, `1.4889` だった。hook rel err は主因ではなく，現状 `n>=4` は Phase3/4 training candidate として不安定である。body deformation 自動判定は初回実行では `body_diag_missing` だったため，generic multi-run で長時間 body diagnostics を明示出力するようにし，再実行後に確認する。36条件 replay は1枚では見づらいため，`replay.max_panels_per_grid` により `grid_swim3d_page*.mp4` へ分割する。
