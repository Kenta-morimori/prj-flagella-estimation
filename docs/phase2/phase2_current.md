...
* #97 では `root_torque_segment_couples + diffusive` を後方主採用候補，`root_torque_axis_projection + uniform` を参照候補として扱う。比較面は `root_torque_segment_couples / root_torque_axis_projection` × `diffusive / uniform` に限定し，`body` 側の反トルクの掛け方は別issueで扱う。#100 の実装 branch では現役 user-facing sweep/heatmap 導線を `shape_stability_grid` / `torque_distribution_grid.yaml` へ移した。#100 の stacked PR を `feature/phase2-97-torque-distribution-review` へ merge した後，比較条件・既存 output contract を変えずに，posterior 1.0 s，lateral 0.6 s，replay qualitative review，最終採用判断へ戻る。
