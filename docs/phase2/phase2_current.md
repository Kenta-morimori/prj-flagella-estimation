...
* Phase 2.8 / Issue #113: dataset v0 で見えた `n>=4` の破綻境界確認に向け，`conf/phase2_multi_run/flagella_count_failure_boundary_seed00.yaml` を追加した。まず `attach_seed=0`, `phase_seed=0`, `n_flagella=4,5,6` を固定条件で実行し，`summary.csv` の `first_fail_t_s` / `first_fail_category_nonbody` / flag・hook指標 / body shape gate 指標と，near-square 化された `grid_swim3d.mp4` replay で破綻種別を確認する。長時間本実行と目視評価はユーザー実行待ちであり，結果次第で `n=7,8,9` へ拡張する。
