...
* Phase 2.8 / Issue #71: 診断用 dataset v0 として，`conf/phase2_multi_run/flagella_count_behavior_diagnostic.yaml` を使い，`n_flagella=1,2,3,6`，`attach_seed=[0,1,2]`，`phase_seed=[0,1,2]`，RUN固定，`duration_s=1.0`，`motor.torque_Nm=2.0e-20` で特徴量分布を確認する。これは Phase3/4 本番訓練 dataset の凍結ではなく，Ground Truth 付きで本数差が `cell_mean_speed`，`cell_straightness`，`cell_wobble`，`flagella_axis_alignment` などへ出るかを見る探索runである。結果を見て，seed数を増やすか，評価時間を伸ばすか，モデル改善へ戻るかを判断する。
* Phase 2.8 / Issue #113: dataset v0 で見えた `n>=4` の破綻境界確認に向け，PR #114 で `conf/phase2_multi_run/flagella_count_failure_boundary_seed00.yaml` と paged replay 出力を追加している。`attach_seed=0`, `phase_seed=0`, `n_flagella=4,5,6` の初回実行結果では全条件が `flag` first fail となり，first fail は `n=4: 0.3273 s`, `n=5: 0.3168 s`, `n=6: 0.3505 s`，max flag bond rel err は `1.8486`, `1.5037`, `1.4889` だった。hook rel err は主因ではなく，現状 `n>=4` は Phase3/4 training candidate として不安定である。PR #114 は Draft / review待ちであり，merge前に body diagnostics 修正後の `n=4,5,6` 再実行，`body_shape_pass` / `body_fail_category` 確認，`grid_swim3d_page*.mp4` と seed固定 replay の目視確認を残す。
* Phase 2.8 / Issue #113 follow-up ordering: #113 / PR #114 を完了して failure mode を確定した後，#117 で現状モデルの diagnostic dataset v0 統計をまとめ，#115 で `n>=4` の flag bond 過伸長をモデル修正する。#118 の `model_id` ベース命名規則は #117 と並行可能だが，#119 の改善モデル dataset v1 再生成より前に完了させる。#116 の少数sweep / heatmap 方針は #115 の修正候補が出てから実施する。#17 は背景ノイズ関連のためこの関係整理から除外する。
