...
* Phase 2 / Issue #97: 後方条件での `body / hook / flagella / attach-bead` 回転安定性を主目的に，トルク分散比較を拡張した。`motor.torque_distribution_profile` を導入し，旧 `motor.torque_segment_weight_profile` は deprecated alias とした。旧 `local_twist_activity` 系 profile は `diffusive`, `diffusive_sqrt`, `diffusive_floor_0p2`, `diffusive_floor_0p4` に改名し，`basal_unloading` を追加した。`root_torque_axis_projection` でも同じ profile 名を使えるようにし，新しい比較方式 `root_torque_hybrid_couples` を追加した。`hook_overstretch` の `torque-profile-grid` は `force_distribution x torque_distribution_profile` を同じ `summary.csv` で比較できる。後方主評価 Stage E `outputs/phase2_97/stage_e_distribution_grid_fp3_ft1p5_torque2p0_dur0p6/summary.csv` では `root_torque_segment_couples + diffusive` だけが 0.6 s を shape pass し，`axis_projection_uniform` は次点だが `t=0.4593 s` で `flag` fail，hybrid 3条件は即時破綻した。9条件の3D定性比較を揃えるため，一時スクリプト `scripts/01_simulate_swimming/render_issue97_grid_qualitative.py` を追加し，3x3 subplot movie を再生成できるようにした。
