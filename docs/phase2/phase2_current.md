...
* #71 では #72/#73 のdataset・plot出力を使い，body displacement, speed, body axis angle change, angular velocity, wobble RMS, flag helix axis alignment を本数別に比較する必要がある。標準datasetは `seed=0` 固定で，各本数1 sampleのため統計的結論ではなく次条件選定の探索結果として扱う。
