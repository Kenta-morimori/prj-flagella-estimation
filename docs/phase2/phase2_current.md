...
* Phase 2 / Issue #97 initial: Issue本文を #94/#95 の結果に基づいて完成し，初期実装として螺旋軸中心診断と torque segment weight 比較導線を追加した。`step_summary.csv` には `flag_helix_axis_center_radius_mean_um`, `flag_helix_axis_center_radius_cv_*`, `flag_helix_axis_center_spin_fit_r2_min`, `flag_helix_axis_center_root_offset_um_*` を出し，`flag_helix_axis_diagnostics.csv` には per-flag の `axis_center_*` 列を出す。`motor.torque_segment_weight_profile` は default `local_twist_activity` のまま，Issue #97 比較用に `uniform` を指定できる。`hook_overstretch` sweep には `mode=torque-profile-grid` を追加し，`fp=3, ft=1.5` などを固定して `local_twist_activity` と `uniform` を同じ `summary.csv` で比較できる。0.001 s smoke では両 profile とも shape pass し，新しい軸中心指標が summary に出ることを確認した。0.6 s 後方条件での本評価と動画目視は未実施であり，#97 の残タスクとして扱う。
...
* #97 では `uniform` torque segment weight profile を比較候補として追加したが，default は `local_twist_activity` のまま維持する。default変更，0.6 s代表条件採用，または物理モデル解釈の変更には，0.6 s 後方条件の自動 gate，必要な目視レビュー，ADR判断が必要である。
