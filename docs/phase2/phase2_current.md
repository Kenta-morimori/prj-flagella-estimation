...
* #97 の 2x2 torque distribution 候補は，後方 1.0 s で全条件 `flag` fail，側方 0.6 s で剛体回転様の user定性評価となったため採用しない。#100 の generic sweep/heatmap 導線は #99 に取り込み済みで，#97/#100 は PR #99 merge で close 対象とする。次の blocker は #103 で，attach-frame補強が hook過伸長を抑える一方で basal/root と body の相対自由度を消していないかを切り分ける。
