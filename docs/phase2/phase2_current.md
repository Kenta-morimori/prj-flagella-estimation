...
* Phase 2 / Issue #97: Issue本文を #94/#95 の結果に基づいて完成し，螺旋軸中心診断と torque segment weight 比較導線を追加した。`step_summary.csv` には `flag_helix_axis_center_radius_mean_um`, `flag_helix_axis_center_radius_cv_*`, `flag_helix_axis_center_spin_fit_r2_min`, `flag_helix_axis_center_root_offset_um_*` を出し，`flag_helix_axis_diagnostics.csv` には per-flag の `axis_center_*` 列を出す。`hook_overstretch` sweep summary には `axis_center_net_abs_revolutions_*` と `axis_center_direction_consistency_*` を集約する。`motor.torque_segment_weight_profile` は default `local_twist_activity` のまま，Issue #97 比較用に `activity_sqrt`, `activity_floor_0p2`, `activity_floor_0p4`, `uniform` を指定できる。0.6 s 後方条件 `fp=3, ft=1.5, torque=2.0e-20` の Stage C では `local_twist_activity` のみ `final_shape_pass_nonbody=True` で，追加4候補はすべて `flag` fail した。追加候補は axis-center 回転量をやや増やすが `max_flag_bond_rel_err` を悪化させるため採用せず，default は `local_twist_activity` に据え置く。`local_twist_activity` の代表動画はユーザー定性評価でOKだった。
...
* #97 の追加探索では `activity_sqrt`, `activity_floor_0p2`, `activity_floor_0p4`, `uniform` が 0.6 s 後方条件でいずれも `flag` fail したため，default は `local_twist_activity` のまま維持する。今後 default変更や物理モデル解釈変更を行う場合は，別issueで複数seed・長時間評価とADR要否を再検討する。
