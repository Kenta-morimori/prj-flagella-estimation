...
* Phase 2.8 / Issue #71: 次は診断用 dataset v0 として，`conf/phase2_analysis/flagella_count_behavior_dataset_torque2p0.yaml` を使い，`n_flagella=1,2,3,6`，`attach_seeds=[0,1,2]`，`phase_seeds=[0,1,2]`，RUN固定，`duration_s=0.5`，`motor.torque_Nm=2.0e-20` で特徴量分布を確認する。これは Phase3/4 本番訓練 dataset の凍結ではなく，Ground Truth 付きで本数差が `cell_mean_speed`，`cell_straightness`，`cell_wobble`，`flagella_axis_alignment` などへ出るかを見る探索runである。結果を見て，seed数を増やすか，評価時間を伸ばすか，モデル改善へ戻るかを判断する。
