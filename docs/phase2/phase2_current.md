...
* Phase 2 / Issue #103: attach-frame補強による剛体回転を診断するため，`body_roll_phase_deg` と `axis_center_body_relative_phase_deg` を追加し，`shape_stability_grid` summary で body roll と螺旋軸中心 spin を分離できるようにした。`motor.local_attach_frame_tangent_mode=basal_bearing` は，`attach -> first` 軸まわり方位角を拘束せず，`first -> second` の軸方向成分と垂直半径だけを保つ #103 診断用 extension である。標準比較 profile は `conf/phase2_sweeps/basal_freedom_diagnostic.yaml` で，`no_frame`, `fp3`, `ft1p5`, `fp3_ft1p5_vector`, `fp3_ft1p5_bearing` の5条件を展開する。長時間 sweep / replay render はユーザー実行待ちである。
...
* #97 の 2x2 torque distribution 候補は，後方 1.0 s で全条件 `flag` fail，側方 0.6 s で剛体回転様の user定性評価となったため採用しない。次の blocker は #103 の評価で，`basal_freedom_diagnostic.yaml` の側方0.6 s / 後方1.0 sをユーザー側で実行し，`fp`, `ft`, `basal_bearing` のどれが body-root 一体化を緩和するかを判断する。
