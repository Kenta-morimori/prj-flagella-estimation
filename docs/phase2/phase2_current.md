...
* Phase 2→3 / Issue #126: dataset v1 の `n_flagella=1,2,3` について，raw `trajectory.csv` からXY投影特徴量を抽出する `scripts/02_phase2_analysis/analyze_2d_separability.py` を追加した。27 sample の grouped nearest-centroid baseline は `26/27 = 0.963` で，2D投影後も本数差は初期指標上残っている。ただし画像frame由来特徴と短いclip窓での識別性は未評価であり，#129 へ接続する。
...
* #126 の初期2D特徴量評価では差が残ったが，これは `trajectory.csv` のXY投影に基づく。実際のpseudo-microscopy画像からの検出・短時間clip化・grouped splitは #127 / #129 で引き続き設計する。
