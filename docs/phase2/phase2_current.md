...
* Phase 2 / Issue #167: Kong et al. (2015)を参考にした `hook_coupled_body_reaction` を実装した。flagellum基部3 beadsへzero-net-force駆動torqueを与え、attach beadとbody ring/vertical one-ringへ逆torqueを与える。局所body supportが縮退した場合だけ全body beadsへfallbackする。これは `paper_inspired_approximation` であり、manifestは `dynamics: implemented`、`geometry: pending`、`simulation: blocked` とreaction model、support bead数、fallback使用有無を区別する。2015 profilesは#166 refined geometryと#168 stability evaluationが未完了のため `implementation_status: pending` を維持し、実simulationを拒否する。#167では2010三角柱geometry上の短時間smokeだけを行い、120-bead長時間runは#168へ残す。設計判断はADR 0013に記録する。
