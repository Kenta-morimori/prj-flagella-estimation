...
* #82 の attach-frame 補強は hook過伸長を 0.5 s 代表条件で抑えるが，flag bond 側の破綻と長時間安定性は残課題である。出力整理では `step_summary.csv`，`trajectory.csv`，`state_archive.npz`，`manifest.json` を再現用として保持し，途中停止run・重複run・報告に使わない再生成可能な動画/frameだけを削除対象にする。
