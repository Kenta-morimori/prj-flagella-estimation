...
* Phase 2.8 / Issue #119: 改善モデル dataset v1 の canonical config `conf/phase2_multi_run/flagella_count_behavior_v1.yaml` を追加した。v1 は `stiffness_scales.flag_spring=2.25`, `stiffness_scales.body=2.5`, `n=1..4`, `attach_seed=0..2`, `phase_seed=0..2`, `dataset_id=v1` とする。既存 smoke output `outputs/phase2_multi_run/flagella_count_stability_smoke_seed00/summary.csv` では候補値の `n=1,2,3` がすべて `final_shape_pass_nonbody=True`, `body_shape_pass=True`, `first_fail_category_nonbody=none` だった。full v1 36 sample raw generation と dataset build は長時間 run としてユーザー実行待ちであり，#119 は未完了である。
