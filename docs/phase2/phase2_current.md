...
* #97 では `root_torque_segment_couples + diffusive` を後方主採用候補，`root_torque_axis_projection + uniform` を参照候補として扱う。比較面は `root_torque_segment_couples / root_torque_axis_projection` × `diffusive / uniform` に限定し，`body` 側の反トルクの掛け方は別issueで扱う。
