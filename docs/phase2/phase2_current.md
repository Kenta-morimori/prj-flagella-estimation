...
* Phase 2.8 / Issue #65: RUN固定状態で，べん毛本数による菌体推進・姿勢安定性・軸整列の違いを評価する。
* Phase 2.9 / Issue #69: Tumble状態を段階実装する。まず設計・診断，次にmotor reversal，polymorph切替，run-and-tumble評価へ分ける。

## Current blockers

* P2-7代表条件は `hook_wrapped_axis_aligned` であり，hook length fail は残る。RUN本数差評価では前提リスクとして扱う。
* #65 では body displacement, speed, body axis angle change, angular velocity, wobble RMS, flag helix axis alignment を本数別に比較する必要がある。
