...
* Phase 2 / Issue #97: 後方条件での `body / hook / flagella / attach-bead` 回転安定性を主目的に，トルク分散比較を整理した。`motor.torque_distribution_profile` を導入し，旧 `motor.torque_segment_weight_profile` は deprecated alias とした。旧 `local_twist_activity` 系 profile は `diffusive`, `diffusive_sqrt`, `diffusive_floor_0p2`, `diffusive_floor_0p4` に改名し，`root_torque_axis_projection` でも同じ profile 名を使えるようにした。後方主評価 Stage E `outputs/phase2_97/stage_e_distribution_grid_fp3_ft1p5_torque2p0_dur0p6/summary.csv` では `root_torque_segment_couples + diffusive` だけが 0.6 s を shape pass し，`axis_projection_uniform` は次点だが `t=0.4593 s` で `flag` fail だった。これを受けて，`root_torque_hybrid_couples` は即時破綻，`basal_unloading` は有効性不足として本線比較から除外し，Issue #97 の標準比較は `root_torque_segment_couples / root_torque_axis_projection` × `diffusive / uniform` の 2x2 に縮小した。`hook_overstretch` sweep は既定で `state_archive.npz` と `trajectory.csv` を保存し，定量 sweep 後に replay/render できるようにした。一時スクリプト `scripts/01_simulate_swimming/render_issue97_grid_qualitative.py` は再シミュレーションせず，sweep 出力 directory を入力として `plot-only / render-only / both` を切り替える replay utility に更新済み。加えて，sweep/heatmap 導線の task-specific naming 依存を外す follow-up を Issue #100 として分離し，#97 はその整理が入るまで block する。#100 の作業 branch は `feature/phase2-100-generic-sweep-cli`，PR base は `feature/phase2-97-torque-distribution-review` とし，そのマージ後に #97 は 1.0 s posterior 4条件定量，比較対象を固定した 0.6 s lateral 参照，比較 output の replay 定性確認，採用条件の最終判断の順で再開する。
...
* #97 では `root_torque_segment_couples + diffusive` を後方主採用候補，`root_torque_axis_projection + uniform` を参照候補として扱う。比較面は `root_torque_segment_couples / root_torque_axis_projection` × `diffusive / uniform` に限定し，`body` 側の反トルクの掛け方は別issueで扱う。あわせて，現役 user-facing sweep/heatmap 導線から `hook_overstretch` 命名を外す Issue #100 を先行 blocker とし，その PR は `feature/phase2-97-torque-distribution-review` base の stacked PR として進める。Issue #100 の branch 名は `feature/phase2-100-generic-sweep-cli` とし，merge 後の #97 再開では比較条件・既存 output contract を変えずに，posterior 1.0 s，lateral 0.6 s，replay qualitative review，最終採用判断へ戻る。
