...
本メモでは、現行 bead-position-only model に不足している状態量を整理し、P2-6-007で実装した probe の結果と、次タスクで扱う完全物理実装の方針を分けて記録する。
...
したがって、本PRは「probe成功」までを完了範囲とする。完全物理実装は、material frame / segment twist / 局所 force couple を導入する次タスクへ分離する。
...
- 次タスク: 完全物理実装では material frame / segment twist を導入し、非局所force injectionを避ける。
...
1. 本PR: `local_twist_transmission_probe` を Phase 2.6 の有効probeとし、`triplet` baseline、`axial_torque_flux_probe`、`distributed_flagellum` upper-bound と比較する。
2. 次タスク: material frame / segment twist を追加し、twist potential から局所 force couple を導出する。
3. 採用判断: 完全物理実装では、単一べん毛で `dt_star=1.0e-4`, 0.5 s以上, net 1回転以上, shape gate PASS に加え、非局所force injectionを使わないことを条件にする。

probe の実装結果は `docs/adr/0002_phase2_torque_transmission_probes.md` にまとめる。完全物理実装の方針は `docs/adr/0004_phase2_material_frame_twist_transmission.md` に記録する。
