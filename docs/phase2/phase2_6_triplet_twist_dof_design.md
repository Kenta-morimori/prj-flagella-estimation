...
| local_twist_transmission_probe | segment orientation state を持ち、root側のorientation activityを先端側へ拡散・緩和させ、そのactivityをbead接線力の重みに使う | 中。完全なmaterial frameではないが、内部twist状態を持つ | rootから先端への内部伝搬を診断できる | bead force変換はまだ近似で、厳密なtwist potential由来ではない | P2-6-007の本命probe |
...
したがって、`distributed_flagellum` と `axial_torque_flux_probe` の二択で比較するなら、`axial_torque_flux_probe` の方が root motor torque の伝搬解釈に近い。ただし、P2-6-007ではさらに segment orientation state を持つ `local_twist_transmission_probe` を実装したため、最終妥協案候補は `local_twist_transmission_probe` を優先する。

## 実装検証

P2-6-007では、`motor.force_distribution=axial_torque_flux_probe` と `motor.force_distribution=local_twist_transmission_probe` を追加した。前者は material frame を明示しない root起点の torque flux probe であり、後者は segment orientation state を持つ local twist probe である。詳細なモデル判断は `docs/adr/0002_phase2_axial_torque_flux_probe.md` と `docs/adr/0003_phase2_local_twist_transmission.md` に記録する。
...
- 最終妥協案候補: `distributed_flagellum` ではなく `local_twist_transmission_probe` を優先する。`axial_torque_flux_probe` は短期比較用、`distributed_flagellum` は上限診断として残す。
...
1. 短期: `local_twist_transmission_probe` を Phase 2.6 の有効candidateとし、`triplet` baseline、`axial_torque_flux_probe`、`distributed_flagellum` upper-bound と比較する。
2. 中期: `local_twist_transmission_probe` のbead force変換を、より厳密なtwist potential由来のforce coupleへ近づける。
